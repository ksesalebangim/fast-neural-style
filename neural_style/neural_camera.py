...
def stylize_camera(args):
    ...
    try:
        while rval:
            ...
            frame = frame[...,[2,1,0]]
            if first:
                content_image = torch.from_numpy(np.transpose(frame, (2,0,1))).unsqueeze(0).float()
                if args.cuda:
                ...
                if args.cuda:
                    output = output.clone().cpu().clamp(0,255).numpy()
                else:
            ...
            cv2.imshow("preview", output.transpose())
    finally:
